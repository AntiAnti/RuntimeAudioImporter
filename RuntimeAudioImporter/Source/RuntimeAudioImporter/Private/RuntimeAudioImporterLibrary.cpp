...
{
	SoundWaveRef->RawPCMDataSize = SoundWaveRef->RawPCMDataSize;
	SoundWaveRef->RawPCMData = (uint8*)FMemory::Malloc(SoundWaveRef->RawPCMDataSize);
	FMemory::Memcpy(SoundWaveRef->RawPCMData, TranscodingFillInfo.PCMInfo.PCMData, SoundWaveRef->RawPCMDataSize);
	/*
	SoundWaveRef->RawPCMData = static_cast<uint8*>(FMemory::Malloc(SoundWaveRef->RawPCMDataSize));
	FMemory::Memmove(SoundWaveRef->RawPCMData, TranscodingFillInfo.PCMInfo.PCMData,
	...
	case EAudioFormat::Mp3:
		{
			...
			TranscodingFillInfo.PCMInfo.PCMData = static_cast<uint8*>(FMemory::Malloc(
				drmp3_get_pcm_frame_count(&mp3) * mp3.channels * sizeof(uint16)));

			// OnProgress Callback
			OnProgress_Internal(35);

			// Filling PCM data and getting the number of frames
			TranscodingFillInfo.PCMInfo.PCMNumOfFrames = drmp3_read_pcm_frames_s16(
				&mp3, drmp3_get_pcm_frame_count(&mp3), reinterpret_cast<drmp3_int16*>(TranscodingFillInfo.PCMInfo.PCMData));
