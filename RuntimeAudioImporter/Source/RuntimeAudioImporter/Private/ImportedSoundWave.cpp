...
int32 UImportedSoundWave::OnGeneratePCMAudio(TArray<uint8>& OutAudio, int32 NumSamples)
{
	...
	const uint8* RetrievedPCMData = PCMBufferInfo.PCMData + (CurrentNumOfFrames * NumChannels * sizeof(uint16));
	const int32 RetrievedPCMDataSize = NumSamples * sizeof(uint16);
...
void UImportedSoundWave::GetPCMBuffer(TArray<uint8>& OutPCMData) const
{
	if (PCMBufferInfo.PCMDataSize > 0 && PCMBufferInfo.PCMData != nullptr)
	{
		OutPCMData.SetNumUninitialized(PCMBufferInfo.PCMDataSize);
		FMemory::Memcpy(OutPCMData.GetData(), PCMBufferInfo.PCMData, PCMBufferInfo.PCMDataSize);
	}
}